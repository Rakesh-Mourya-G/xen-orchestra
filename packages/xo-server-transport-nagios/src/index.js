...
Otherwise, you could choose a custom name but the template \`{job.name}\` must e included. For example: \`{job.name}-CloudVisor\`.`
...
class XoServerNagios {
  ...
  test({ VmNameLabel, jobName }) {
    return this._sendPassiveCheck(
      {
        message: 'The server-nagios plugin for CloudVisor server seems to be working fine, nicely done :)',
        status: OK,
      },
      VmNameLabel,
      jobName
